...
const jsonData = async (req, res) => {
    try {
        ...
        if (!jsonInput || !jsonInput.results) {
        ...
        res.status(201).json({ message: 'Data processed successfully', serialNumber: newSerialNumber });
    } catch (error) {
        console.error('Error in jsonData:', error);
